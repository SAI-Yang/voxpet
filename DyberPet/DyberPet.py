...
class PetWidget(QWidget):
    ...
    def _init_widget(self) -> None:
        ...
        # Windows: 彻底移除透明窗口的阴影和边框
        if platform == 'win32':
            try:
                import ctypes
                from ctypes import wintypes
                hwnd = int(self.winId())
                # 禁用 DWM 非客户区渲染（去掉阴影/圆角）
                DWMWA_NCRENDERING_POLICY = 2
                DWMNCRP_DISABLED = 1
                ctypes.windll.dwmapi.DwmSetWindowAttribute(
                    hwnd, DWMWA_NCRENDERING_POLICY,
                    ctypes.byref(ctypes.c_int(DWMNCRP_DISABLED)), ctypes.sizeof(ctypes.c_int))
                # 禁用 DWM 窗口圆角
                DWMWA_WINDOW_CORNER_PREFERENCE = 33
                DWMWCP_DONOTROUND = 1
                ctypes.windll.dwmapi.DwmSetWindowAttribute(
                    hwnd, DWMWA_WINDOW_CORNER_PREFERENCE,
                    ctypes.byref(ctypes.c_int(DWMWCP_DONOTROUND)), ctypes.sizeof(ctypes.c_int))
                # 禁用 DWM 过渡动画
    ...
    def ontop_update(self):
        ...
        self.setAutoFillBackground(False)
        self.setAttribute(Qt.WA_TranslucentBackground, True)
        self.show()  # showEvent 会处理 Windows DWM 修复
    ...
    def _init_ui(self):
        ...
        self.status_frame.setAttribute(Qt.WA_TranslucentBackground, True)
        self.status_frame.setStyleSheet("background: transparent;")
        vbox = QVBoxLayout()
    ...
    def _set_Statusmenu(self):
        ...
        # Voice Assistant
        self.voice_act = Action(self.tr('🎤 Voice Assistant'), triggered=self._toggle_voice)
        self.StatMenu.addAction(self.voice_act)
        self.StatMenu.addSeparator()
    ...
    def _toggle_voice(self):
        ...
        # 更新菜单文字反映状态
        if va._recording:
            self.voice_act.setText(self.tr('⏹ Stop Recording'))
        else:
            self.voice_act.setText(self.tr('🎤 Voice Assistant'))
